...
const WordToPDF: React.FC = () => {
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && (file.type.includes('document') || file.name.endsWith('.docx') || file.name.endsWith('.doc'))) {
  ...
  const downloadConvertedFile = () => {
    if (convertedFileUrl && selectedFile) {
      ...
      a.download = `${selectedFile.name.replace(/\.(docx?|rtf)$/i, '')}.pdf`;
      a.click();
    }
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-8">
      ...
      <p className="text-gray-600 mb-8">
        Convert your Word documents to PDF format. Perfect for sharing and preserving document formatting.
      </p>
          ...
          >
            {selectedFile ? (
              ...
              <>
                ...
                <p className="text-gray-600">Click to upload a Word document</p>
                <p className="text-sm text-gray-400 mt-2">Supports .docx, .doc, .rtf files</p>
              </>
            )}
          </div>
